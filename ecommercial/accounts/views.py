...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid(): # to check whether all the field in this form is valid or not. 
            ...
            user = Account.objects.create_user(first_name = first_name, last_name = last_name, email = email, password=password) # there is no field to accept phone number in models.py so we are attaching it like.
